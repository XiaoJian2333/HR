...
<script type="text/javascript">
	...
	function ckmxStaff(){												//查看明细按钮
		...
		$("#dlg").dialog("open").dialog("setTitle","查看薪酬标准");
		$("#fm").form("load",row);
		$.ajax({
            type: "post",
            url: "standard?add=1",
            ...
            success: function (result) {
            	...
            	$.each(result,function(idx, obj){
            		...
            		$("#i_table").append("<tr>"+"<tr><td colspan='2'>——————————</td></tr>"+
            							      ...
            							      "<td colspan='2'>"+"<input type='text' id='salary"+obj.itemid+"' name='salary' class='easyui-numberbox' data-options='min:0,precision:2,value:0.00' readonly value='"+obj.salary+"'  /></td>"+
            							      "</tr>");            		
            	$(function(){
	...
	function openAddDialog(){								//添加按钮
		$("#dlg").dialog("open").dialog("setTitle","添加薪酬标准");
	...
	function openStaffModifyDialog(){								//修改按钮
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length!=1){
		...
		var row=selectedRows[0];
		$("#dlg").dialog("open").dialog("setTitle","编辑薪酬标准");
		$("#fm").form("load",row);
		$.ajax({
            type: "post",
            url: "standard?add=1",
		...
		$("#positionId").combobox('readonly',true);
		url="standard?add=4&id="+row.id+"&state=1";
	}
	function f(){
		$("#flag").nextAll().remove();
		$.ajax({
	 ...
	 pagination="true" rownumbers="true" url="standard?add=3" fit="true" toolbar="#tb">
		<thead>
			<tr>
				...
				<th field="positionId" width="150" align="center" hidden="true">职位ID</th>
